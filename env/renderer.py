...
class FSRenderer:
    ...
    def render(self) -> np.ndarray:
        ...
        # Raio adaptativo: garante visibilidade em pistas grandes (YAML)
        cone_r = max(3, int(0.3 * self.scale))
        orange_r = max(4, int(0.45 * self.scale))
        self._draw_cones_with_status(
            td['blue_cones'], self.BLUE_CONE, cone_r,
            self.env.knocked_blue
        )
        self._draw_cones_with_status(
            td['yellow_cones'], self.YELLOW_CONE, cone_r,
            self.env.knocked_yellow
        )
        self._draw_cones_with_status(
            td['orange_cones'], self.ORANGE_CONE, orange_r,
    ...
    def _draw_car(self):
        ...
        pygame.draw.circle(self.screen, self.CAR_FRONT, fp, max(3, int(0.2 * self.scale)))
